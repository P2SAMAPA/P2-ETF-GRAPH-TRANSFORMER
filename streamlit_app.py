"""
Streamlit Dashboard for Graph Transformer Engine.
"""
...
st.set_page_config(page_title="P2Quant Graph Transformer", page_icon="🔗", layout="wide")
...
def load_latest_results():
    try:
        ...
        json_files = sorted([f for f in files if f.startswith("graph_transformer_") and f.endswith('.json')], reverse=True)
        if not json_files:
            return None
...
st.markdown('<div class="main-header">🔗 P2Quant Graph Transformer</div>', unsafe_allow_html=True)
st.markdown('<div>Graph Transformer – Global Attention across ETFs</div>', unsafe_allow_html=True)
